...
% ============================================================
\begin{abstract}
Iterative self-improvement methods assume that a model can reliably judge the
correctness of its own outputs.
We test this assumption by tracking the \emph{verification gap}
$\Delta_t = \bar{s}_t^{\text{self}} - \bar{s}_t^{\text{ext}}$ over 20
iterations of self-supervised fine-tuning on the MATH benchmark.
Across three model families (Qwen2.5-7B, Llama-3-8B, Mistral-7B), the gap
grows monotonically: self-assigned scores inflate while ground-truth accuracy
remains flat, a failure mode we call \emph{verification collapse}.
The model does not merely become overconfident; the corrupted self-score
prevents genuine improvement by training the model on its own wrong answers.
A GT-anchored control, identical except that training examples are filtered
by ground truth rather than self-score, eliminates collapse and improves
accuracy by 78\%, isolating self-scoring as the causal driver.
We propose \emph{adversarial hard-negative injection}: periodically replacing
a fraction of the training batch with the model's most confident errors paired
with gold solutions.
Using 85\% less ground-truth data than full GT supervision, injection reduces
the steady-state gap by 47\% while reaching comparable training accuracy
(0.525 vs.\ 0.535), demonstrating that a small, targeted dose of ground truth
on the model's worst failures captures most of the benefit of continuous GT
supervision.
We study collapse in a verifiable domain precisely because it is measurable
there; in non-verifiable domains, the same drift would be invisible and
unchecked.
\end{abstract}
...
%   Iterative self-improvement (STaR, SRLM, RLHF) relies on the model's
...
%   (iii) We situate the result relative to SRLM, STaR, and RM overoptimization
...
Methods ranging from STaR \citep{zelikman2022star} and Self-Rewarding Language
Models \citep{yuan2024self} to Absolute Zero Reasoner \citep{zhao2025azr}
share a common design: the model generates a candidate answer, evaluates it,
and uses that evaluation to select or improve training examples.
Some of these systems retain a ground-truth anchor to varying degrees: STaR
filters on ground-truth correctness, and AZR delegates verification to a code
executor.
But Self-Rewarding Language Models represent the most closed case: the model
assigns its own reward signal at every iteration, with no per-example
ground-truth label during self-improvement.
...
own reward signal at every iteration; no external correctness signal enters the
self-improvement loop.
In iterative STaR variants, the model generates rationales that are filtered by
ground truth.
Where the anchor is the model's own judgment rather than an external signal,
self-verification is not a peripheral component: it is the mechanism that
converts generation into a training signal.
...
reasoning errors without an external oracle: absent ground-truth feedback,
models revise correct answers as readily as incorrect ones, causing net
performance degradation.
Concurrent work documents growing calibration error across iterative
self-improvement rounds \citep{huang2025beyondaccuracy,epicar2026}, and
\citet{rlsr2025} observe self-reward diverging from formal reward under RL
pressure.
Yet a precise, dynamic characterisation of this failure mode, specifically how the gap
...
Calibration Error, AUROC, Brier Score) over short horizons of three to five rounds; no
...
The question this raises is precise: \emph{does overconfidence in self-scoring
prevent genuine improvement in a recursive self-improvement loop, and can
periodically grounding the verifier on ground truth restore it?}
We study this in the MATH benchmark, a verifiable domain where ground truth
exists and miscalibration is directly measurable.
This choice is deliberate: in non-verifiable domains such as instruction
following or open-ended generation, the same drift would be invisible.
Without an external signal, there is no way to detect that self-assigned scores
have decoupled from reality; the loop would simply continue, training on
increasingly wrong evidence with increasing confidence.
We measure the phenomenon where it can be measured, so that its existence in
the broader class of self-improving systems is no longer in doubt.
...
with a causal control condition---a failure mode distinct from reward model
overoptimization \citep{gao2023scaling} and from the filtering-utility gap of
\citet{song2025mindthegap}---across three model families.
...
using 85\% less ground-truth data than full GT supervision, while reaching
comparable training accuracy ($0.525$ vs.\ $0.535$).
It requires no objective modification, no external judge, and no architectural
change, making it applicable to any self-improving system.
Adversarial injection is, in effect, a mechanism for distilling ground-truth
signal into the self-verifier itself: by periodically grounding the model on
examples where GT and self-score disagree most sharply, we restore calibration
without replacing the self-improving loop.
...
In domains without such an anchor, for instance open-ended reasoning or instruction
following, the loop must rely on the model's own judgment, and whether that judgment
remains calibrated across training has not been tested empirically; the present work
provides the test, and it fails.
...
Song et al.\ track the GV-Gap, a derived quantity of verification scores, at every
iteration, but this measures whether verification improves \emph{filtering utility}, not
whether raw self-assigned scores remain calibrated to ground truth.
A model that grows monotonically overconfident across all 20 iterations could be
consistent with a GV-Gap of zero.
...
judge.
\citet{sun2025solververifier} provide a theoretical complement: they model
solver and verifier capability as converging exponentially to stable limits under
iterative self-improvement.
Our empirical results document the pathological case their theory does not cover:
verifier confidence inflates monotonically while solver accuracy stagnates, a
...
\citet{huang2025beyondaccuracy} (\emph{Beyond Accuracy}) measure Expected Calibration Error (ECE),
...
Flash Attention 2 \citep{dao2023flashattention2} and a LoRA adapter
...
\midrule
1  & 0.465 & 0.360 & 0.400 & 0.105 & 0.0624 & 25 \\
5  & 0.620 & 0.400 & 0.420 & 0.220 & 0.0184 & 47 \\
10 & 0.675 & 0.425 & 0.440 & 0.250 & 0.0056 & 52 \\
15 & 0.750 & 0.455 & 0.440 & 0.295 & 0.0033 & 62 \\
20 & 0.770 & 0.450 & 0.460 & 0.320 & 0.0010 & 67 \\
\bottomrule
\end{tabular}
\end{table}
...
genuine accuracy gains, while adversarial injection, using 85\% less ground-truth-grounded data
than the GT-anchored condition (6 of 20 iterations, 50\% of batch),
reduces the steady-state gap by 47\% and reaches comparable training
accuracy ($\overline{\gt}_{\text{train},20} = 0.525$ vs.\ $0.535$
GT-anchored).
...
Four ablations probe the scope and mechanism of these findings.

% --- WRITING HOOK ---
%
% Fill \todo{} placeholders once pods finish (~April 10-11).
% random_negatives and llama3_injection are running now.
% baseline_500 and recovery follow on Pod 1 after random_negatives.
%
% Key questions each ablation answers:
%   random_negatives  — is overconfidence-ranking load-bearing, or does any GT data suffice?
%   llama3_injection  — does injection generalise across model families?
%   baseline_500      — does collapse replicate at 500 samples?
%   recovery          — is injection corrective, or only preventive?
...
from the \textsc{baseline-500} iter-20 checkpoint.
}
\label{fig:ablations}
\end{figure}
...
The \textsc{random-negatives} ablation isolates them.
...
\begin{tabular}{lcccc}
\toprule
Condition & $\gap_1$ & $\gap_{20}$ (steady) & $\overline{\gt}_\text{train,20}$ & $\overline{\gt}_\text{val,20}$ \\
\midrule
Baseline (self-score)             & 0.105 & 0.320 & 0.450 & 0.460 \\
Injection ($\theta{=}0.7$, 50\%)  & 0.105 & 0.170 & 0.525 & 0.420 \\
Random negatives ($\theta{=}0$)   & 0.173 & 0.095 & 0.410 & 0.350 \\
\bottomrule
\end{tabular}
\end{table}
...
examples (236--326 per iteration), whereas hard-negative selection
($\theta = 0.7$) restricts to a smaller, more homogeneous subset of
high-confidence errors (22--84).
...
The \textsc{llama3-injection} run applies the same injection protocol
...
(iterations~4--10, where the injection run's non-injection gaps are 25--48\%
...
iteration~20.
The baseline undergoes its natural self-judgment degradation and partial
recovery after iteration~13; the injection run does not, because the periodic
hard-negative injections sustain the model's capacity for confident
(but wrong) self-scoring, preventing the LoRA-distortion-driven recovery
that occurs without intervention.
Injection effectiveness is thus capability-gated: on Qwen2.5-7B
($\bar{\gt}^\text{train}_1 = 0.36$), the model can productively use
the GT recalibration signal; on Llama-3-8B ($\bar{\gt}^\text{train}_1 = 0.23$),
the same signal is overwhelmed by the volume of overconfident self-judged
examples in the non-injection portion of each batch.
This establishes a boundary condition for the method: injection requires
sufficient base competence to benefit from ground-truth correction.

\subsection{Scale Replication: Baseline at 500 Samples}
\label{sec:baseline-500}

The primary baseline uses 250 training problems (200 train / 50 val) drawn from MATH L3--5.
To rule out the possibility that collapse is an artifact of training set size---for example,
if the gap grows because the model rapidly memorises a small problem pool---we replicate
the baseline at 500 samples (400 train / 100 val).
The \textsc{baseline-500} run uses the same model (Qwen2.5-7B-Instruct), loop, filter
(\texttt{self\_score}), and iteration count (20) as the primary baseline.

\begin{table}[h]
\centering
\caption{Baseline replication at 500 samples vs.\ primary baseline (250 samples).}
\label{tab:baseline-500}
\small
\begin{tabular}{lcccc}
\toprule
...
Collapse replicates at 500 samples with nearly identical trajectory to the 250-sample
primary baseline.
The gap grows from $0.173$ at iteration~1 to $0.320$ at iteration~20, matching the
primary baseline's final gap of $0.320$ exactly, while $\bar{\gt}^\text{train}$
ends at $0.375$ versus $0.450$ in the 250-sample run---a modest difference attributable
to the larger, more diverse training pool.
...
Small-dataset memorization is therefore not a confound: collapse at 250 samples is not
driven by the model exhausting a small problem pool, but by the self-score filter
mechanism itself.
...
The \textsc{recovery} experiment begins from the \textsc{baseline-500} checkpoint at
iteration~20 ($\gap_{20} = 0.320$)---a fully collapsed
model---and applies 20 further iterations with injection ($N{=}3$, $R{=}50\%$,
$\theta{=}0.7$).
...
The trend is nonetheless clearly downward: by iteration~15 the gap reaches $0.052$
(its lowest point, $\bar{\gt}^\text{train} = 0.425$), before rising again as the
...
By iteration 4, the gap ($\Delta_t$) already exceeds 0.20 in the Qwen baseline,
which means the model is systematically selecting \emph{wrong} completions
as positive training examples.
The LoRA adapter then reinforces the patterns that trigger high
...
only marginally ($0.36 \to 0.45$) while $\bar{\self}$ climbs steeply
($0.465 \to 0.770$) on the back of those memorised completions---a
divergence that is not reflected in real held-out performance, which rises
only modestly from $\gt^\text{val} = 0.40$ to $0.46$ over 20 iterations.
...
This is related to \citet{wang2024learningfromfailure}, who inject failed
trajectories into SFT fine-tuning for LLM agents to improve task performance.
Our setting differs: we target verifier recalibration rather than task
performance, select examples by overconfidence rank rather than task failure,
and operate inside an iterative self-improvement loop rather than a single
fine-tuning pass.
...
to the baseline across iterations (both fluctuate in the 0.38--0.46 range),
...
compared to a full reversal under GT-anchoring ($0.122$).
...
suffice.
The \textsc{random-negatives} ablation (Section~\ref{sec:random-negatives})
...
Injection initially helps---non-injection gaps are 25--48\% below baseline
...
The mechanism is asymmetric: on Qwen, injection resets overconfidence and
$\bar{\gt}^\text{train}$ improves between injection steps; on Llama-3,
the periodic injections sustain the model's capacity for confident self-scoring,
preventing the natural LoRA-distortion-driven recovery that occurs in the
uninjected baseline after iteration~13.
The injection run thus maintains the conditions for overconfidence to
compound, rather than allowing self-judgment to degrade into a less harmful
(if also less useful) low-confidence state.
...
signal.
A rough threshold emerges from our data: injection helps at $\sim$36\%
base accuracy but hurts at $\sim$23\%, suggesting that practitioners should
verify base competence on the target domain before deploying injection as a
mitigation.
...
does not disentangle; the \textsc{random-negatives} ablation addresses this directly.
Third, we ablate the injection interval ($N \in \{1,3,6\}$) and ratio
...
Fourth, our evaluation domain is MATH L3--5 (and L5 only for the difficulty
...
while using 85\% less ground-truth data than full GT supervision, reaching
comparable training accuracy ($0.525$ vs.\ $0.535$).
Injection works by distilling ground-truth signal directly into the
self-verifier: a small, targeted dose of ground truth applied to the model's
most confident mistakes captures most of the benefit of continuous GT
supervision, without replacing the self-improving loop.
Moreover, injection is not only preventive but corrective: applied to a
model that has already undergone 20 iterations of unchecked collapse
...
instrument $\Delta_t$ explicitly, and any system that aspires to operate in
non-verifiable domains should ask whether its self-verifier has been grounded
in domains where ground truth is available.
A confidently wrong self-verifier, left unchecked, does not merely stall
improvement; it actively trains the model away from correctness, silently,
at every iteration.
