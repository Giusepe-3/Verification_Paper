...
\AddEverypageHook{%
  \ifnum\value{page}=1
    \put(-42,-340){\rotatebox{90}{\normalsize\sffamily\textcolor{gray}{Preprint.}}}%
  \fi
}
...
\author{%
  ...
  \texttt{leonardogianola709@gmail.com}%
}
...
% ============================================================
\begin{abstract}
Closed-loop self-improvement methods such as Self-Rewarding Language Models,
Meta-Rewarding, and CREAM dispense with external correctness entirely, using the
model as its own sole judge.
This design rests on a fragile assumption: that self-evaluation remains
calibrated across training iterations.
Tracking the \emph{verification gap}
$\Delta_t = \bar{s}_t^{\text{self}} - \bar{s}_t^{\text{ext}}$ across 20
iterations of self-supervised fine-tuning on the MATH benchmark with three
model families, we find that the gap grows in every case while
ground-truth accuracy remains flat or improves only modestly, a failure mode we
call \emph{verification collapse}.
Three collapse regimes emerge: Qwen2.5-7B drifts gradually ($\Delta_1 = 0.173$,
$\Delta_{20} = 0.320$), Llama-3-8B exhibits transient collapse that peaks and
partially recovers, and Mistral-7B locks in immediately ($\Delta_1 = 0.580$,
$\Delta_{20} = 0.850$).
A GT-anchored control, identical except that training examples are filtered by
ground truth rather than self-score, eliminates collapse entirely, isolating
self-scoring as the causal driver.
Collapse persists under full fine-tuning with all 7.6B parameters trainable
($\Delta_{20} = 0.338$), ruling out adapter-specific artifacts.
We propose \emph{adversarial hard-negative injection}, periodically replacing a
fraction of the training batch with incorrectly solved examples paired with gold
solutions.
On Qwen2.5-7B the intervention reduces the steady-state gap by 47\% using 85\%
less ground-truth data than full GT supervision, and applied to a collapsed
checkpoint it recovers 37\% of the gap, making injection both preventive and
corrective.
Overconfidence ranking is not load-bearing (random negatives perform
equivalently), and the intervention is capability-gated: the same protocol
worsens collapse on Llama-3-8B.
We study collapse in a verifiable domain precisely because it is measurable
there; in non-verifiable settings, analogous drift would be difficult to detect.
\end{abstract}
...
principle, unbounded.
The same design underlies proposals for \emph{scalable oversight}: training
systems to evaluate their own outputs, or those of other models, so that
supervision can extend beyond the limits of human annotation budgets
\citep{bai2022constitutional}.
Whether that extension is safe depends on whether the self-evaluation signal
remains faithful to the quantity it is meant to track, and on whether a silent
failure of that faithfulness can be detected before it degrades downstream
behaviour.
...
distortion: self-scores inflate while ground-truth accuracy improves only
modestly or not at all, silently degrading the quality of every subsequent
training signal.
...
Finally, we use \emph{adversarial hard-negative injection} as a probe of the
collapse mechanism: periodically replacing a fraction of the fine-tuning batch
with incorrectly solved examples paired with gold solutions reduces the
steady-state gap by $47\%$ on a capable model without a persistent external
judge, isolating how much ground-truth signal the loop needs to stay calibrated.
...
the mechanism.
\textbf{(iii)}~We probe the mechanism of collapse using adversarial
hard-negative injection, periodically replacing a fraction of the
training batch with gold solutions for incorrectly solved problems.
Using 85\% less ground-truth data than full GT supervision, injection
bounds the steady-state gap to a 47\% reduction on Qwen2.5-7B;
overconfidence ranking is not load-bearing (random negatives perform
equivalently); and on a weaker model (Llama-3-8B) the same protocol
worsens collapse, establishing a capability threshold for the
intervention.
...
verifier confidence inflates steadily while solver accuracy fails to keep pace,
a divergence that lies outside the convergence regime their model predicts.
...
The results reveal that verification collapse occurs across all three
families tested, in a \emph{model-capability-dependent} manner, exhibiting
three qualitatively distinct trajectory regimes.
...
after 13 such rounds the accumulated parameter drift degrades the
...
$0.368$ (iteration~13) then partially recovers to $0.148$ as iterative
parameter drift degrades self-judgment itself. Ground-truth accuracy remains flat throughout.}
\label{tab:llama3}
\small
\begin{tabular}{rcccccr}
\toprule
...
\midrule
Qwen2.5-7B  & Baseline (LoRA)               & 500 & 0.173 & 0.335 (i19)  & 0.320 & 0.375 \\
...
rather than sustained upward drift), driven by a different failure mode (iterative degradation of
...
iteration~20.
In the uninjected baseline, iterative parameter drift after iteration~13 degrades the
...
entrenched parameter distortions that periodic injection can partially but not fully
overcome.
...
The gap growth pattern is the same gradual upward drift regime observed in the LoRA
...
All results reported above use seed 42.
To verify that the collapse and injection trajectories are not specific to this
choice, we re-run both the baseline and the injection condition at
250 samples on Qwen2.5-7B with seed 43 under otherwise identical hyperparameters.
The pattern reproduces cleanly: the baseline gap grows from
$\gap_1 = 0.130$ to $\gap_{20} = 0.295$ (vs.\ $0.105 \to 0.320$ at seed 42),
and injection bounds it at $\gap_1 = 0.130$, $\gap_{20} = 0.165$
(vs.\ $0.105 \to 0.170$ at seed 42), a 44\% steady-state reduction compared
to 47\% at seed 42 (Appendix~\ref{app:seed43}).
Across the two seeds, the final-gap interval is $[0.295, 0.320]$ for
the baseline and $[0.165, 0.170]$ for injection; the two intervals do not
overlap, so both the existence of collapse and the sign and scale of the
injection effect are stable across seeds.
...
\paragraph{Quantifying filter corruption.}
The damage is visible directly in the false positive rate (FPR) of the
self-score filter: the fraction of admitted training examples that are
actually incorrect, computed as
$\text{FPR}_t = \gap_t / \bar{\self}_t$
(a lower bound, assuming every correct example passes the filter).
Figure~\ref{fig:filter-fpr} plots this quantity across iterations and
conditions.
In the Qwen baseline (500 samples), FPR grows from 35.0\% (iteration~1) to
46.0\% (iteration~20): by the final iteration, nearly half the examples the
model selects for training are wrong. The 250-sample run is steeper still on
the same metric (22.6\% $\to$ 41.6\%), consistent with a smaller pool being
easier to memorise.
Under full fine-tuning the trajectory is comparable to the 500-sample LoRA
baseline (36.9\% $\to$ 47.0\%), confirming this is not an adapter artifact.
Mistral's filter is corrupted from the start (FPR$_1$ = 92.8\%),
consistent with its near-zero base accuracy.
The injection run bounds FPR to 24.5\% at iteration~20, and the
GT-anchored control \emph{reduces} it from 38.8\% to 18.6\%,
demonstrating that external grounding progressively improves filter
quality rather than merely slowing its degradation.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_filter_fpr.pdf}
\caption{\textbf{Self-score filter false positive rate} (FPR$_t =
\gap_t / \bar{\self}_t$) across iterations.
\textbf{(a)}~Qwen baselines under LoRA and full fine-tuning show
FPR climbing toward 42--47\%.
\textbf{(b)}~Injection bounds FPR; GT-anchored control decreases it.
\textbf{(c)}~Mistral's filter is $>$90\% wrong from iteration~1;
Llama-3 peaks at 59\% before partial recovery.}
\label{fig:filter-fpr}
\end{figure}
...
to $\gap_{20} = 0.148$.
We attribute this non-monotonicity to iterative parameter drift eventually degrading
the self-judgment mechanism itself: at high $\Delta_t$, repeated fine-tuning on
wrong examples has overwritten enough of the instruction-following subspace that the model also
...
\paragraph{What injection reveals about the mechanism.}
...
Injecting them into the fine-tuning batch every third iteration (using the
gold MATH solution as the training target) reveals that minimal GT signal
bounds collapse, regardless of how injected examples are selected.
...
Without injection, iterative parameter drift would eventually degrade instruction
...
In instruction following, creative writing, or open-ended reasoning, analogous
drift between self-score and real quality would be difficult to detect: without
a direct external signal, any decoupling of the model's self-assigned scores
from reality has to be inferred indirectly.
...
Several alignment-oriented training pipelines instantiate the same closed-loop
structure studied here: the model (or a near-copy of it) generates candidate
behaviours, judges those candidates against a rubric or constitution, and
trains on the judgments \citep{bai2022constitutional}.
The judge is typically close to the policy, and the judgment target
(helpfulness, harmlessness, reasoning quality) is not closed-form verifiable.
Our results suggest three concrete cautions for this setting.
First, the feedback loop does not need explicit RL pressure to produce
reward-hacking-style divergence; iterative supervised fine-tuning alone is
sufficient (Section~\ref{sec:related}).
Second, the shape and severity of collapse depend on the judge's base
competence on the target task: weaker judges lock in immediately, capable
judges drift slowly but persistently.
Collapse is therefore hardest to notice in precisely the regime most
attractive for deployment, namely a strong model judging a slightly weaker
policy.
Third, because the quantity that collapses is calibration against an external
referent, monitoring schemes that rely on the judge's own confidence,
self-reported accuracy, or win-rate against an older version of itself can
continue to report improvement while $\Delta_t$ grows unchecked.
Detection therefore requires periodic grounding in an external signal,
however narrow, rather than a purely endogenous consistency check.
Our injection result is one such grounding: a small amount of GT exposure
($\sim$15\% of the data used by a fully supervised loop) is enough to bound
the gap on a capable model without an external judge running continuously.
Extending this principle to non-verifiable domains, where the reference
signal is itself produced by humans or by a held-out trusted model, is a
direct avenue for safety-oriented follow-up work.
...
improvement; it poisons the training signal, leaving real performance
nearly flat while self-reported scores rise at every iteration.
The same failure mode is latent in any scalable-oversight pipeline that trains
on self-generated judgments without periodic grounding, and the practical
lesson is narrow but actionable: instrument the self-verifier against an
external referent, even sparingly, and budget for the fact that the feedback
loop will, unchecked, learn to fit its own confabulations.
...
% ============================================================
\appendix
\section{Full Iteration-Level Data}
\label{app:full-data}

Tables~\ref{tab:app-baseline}--\ref{tab:app-full-ft} report the complete
per-iteration metrics for all experiments.
Columns: $\overline{\self}$ = mean self-score (train), $\overline{\gt}_\text{tr}$ = ground-truth
accuracy (train), $\overline{\gt}_\text{val}$ = ground-truth accuracy (held-out val),
$\gap = \overline{\self} - \overline{\gt}_\text{tr}$, HN = hard negatives found,
$\mathcal{L}$ = fine-tuning loss.

% --- Baseline (250) ---
\begin{table}[h]
\centering
\caption{Baseline (Qwen2.5-7B, 250 samples, self-score filter).}
\label{tab:app-baseline}
\small
\begin{tabular}{rcccccc}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.465 & 0.360 & 0.40 & 0.105 & 25  & 0.0624 \\
2  & 0.550 & 0.390 & 0.40 & 0.160 & 33  & 0.0376 \\
3  & 0.565 & 0.410 & 0.40 & 0.155 & 35  & 0.0271 \\
4  & 0.620 & 0.410 & 0.38 & 0.210 & 44  & 0.0195 \\
5  & 0.620 & 0.400 & 0.42 & 0.220 & 47  & 0.0184 \\
6  & 0.655 & 0.430 & 0.36 & 0.225 & 47  & 0.0175 \\
7  & 0.615 & 0.415 & 0.40 & 0.200 & 45  & 0.0131 \\
8  & 0.630 & 0.425 & 0.38 & 0.205 & 44  & 0.0091 \\
9  & 0.670 & 0.430 & 0.44 & 0.240 & 50  & 0.0081 \\
10 & 0.675 & 0.425 & 0.44 & 0.250 & 52  & 0.0056 \\
11 & 0.710 & 0.445 & 0.46 & 0.265 & 55  & 0.0056 \\
12 & 0.725 & 0.440 & 0.44 & 0.285 & 58  & 0.0035 \\
13 & 0.750 & 0.440 & 0.46 & 0.310 & 63  & 0.0041 \\
14 & 0.745 & 0.445 & 0.40 & 0.300 & 62  & 0.0033 \\
15 & 0.750 & 0.455 & 0.44 & 0.295 & 62  & 0.0033 \\
16 & 0.740 & 0.445 & 0.42 & 0.295 & 61  & 0.0022 \\
17 & 0.750 & 0.450 & 0.46 & 0.300 & 63  & 0.0026 \\
18 & 0.755 & 0.440 & 0.44 & 0.315 & 64  & 0.0016 \\
19 & 0.760 & 0.440 & 0.46 & 0.320 & 65  & 0.0018 \\
20 & 0.770 & 0.450 & 0.46 & 0.320 & 67  & 0.0010 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Injection (Qwen2.5-7B, 250 samples, inject every 3 iters, 50\% ratio). $\bigstar$ = injection iteration.}
\label{tab:app-injection}
\small
\begin{tabular}{rccccccl}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ & \\
\midrule
1  & 0.465 & 0.360 & 0.42 & 0.105 & 25  & 0.0624 & \\
2  & 0.515 & 0.405 & 0.38 & 0.110 & 25  & 0.0368 & \\
3  & 0.635 & 0.370 & 0.38 & 0.265 & 56  & 0.1915 & $\bigstar$ \\
4  & 0.510 & 0.425 & 0.40 & 0.085 & 22  & 0.0224 & \\
5  & 0.510 & 0.420 & 0.40 & 0.090 & 24  & 0.0195 & \\
6  & 0.630 & 0.305 & 0.46 & 0.325 & 67  & 0.2140 & $\bigstar$ \\
7  & 0.500 & 0.395 & 0.46 & 0.105 & 24  & 0.0163 & \\
8  & 0.470 & 0.405 & 0.46 & 0.065 & 22  & 0.0192 & \\
9  & 0.680 & 0.270 & 0.46 & 0.410 & 84  & 0.1948 & $\bigstar$ \\
10 & 0.565 & 0.445 & 0.44 & 0.120 & 32  & 0.0207 & \\
11 & 0.590 & 0.435 & 0.40 & 0.155 & 37  & 0.0131 & \\
12 & 0.755 & 0.280 & 0.46 & 0.475 & 97  & 0.1479 & $\bigstar$ \\
13 & 0.585 & 0.450 & 0.46 & 0.135 & 32  & 0.0071 & \\
14 & 0.630 & 0.455 & 0.50 & 0.175 & 38  & 0.0102 & \\
15 & 0.800 & 0.390 & 0.46 & 0.410 & 83  & 0.1234 & $\bigstar$ \\
16 & 0.635 & 0.485 & 0.44 & 0.150 & 35  & 0.0080 & \\
17 & 0.630 & 0.475 & 0.44 & 0.155 & 34  & 0.0111 & \\
18 & 0.810 & 0.445 & 0.46 & 0.365 & 78  & 0.0987 & $\bigstar$ \\
19 & 0.685 & 0.495 & 0.52 & 0.190 & 41  & 0.0105 & \\
20 & 0.695 & 0.525 & 0.42 & 0.170 & 39  & 0.0120 & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{GT-anchored control (Qwen2.5-7B, 500 samples, GT filter).}
\label{tab:app-gt-anchored}
\small
\begin{tabular}{rcccccc}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.490 & 0.300 & 0.37 & 0.190 & 79  & 0.0672 \\
2  & 0.515 & 0.348 & 0.40 & 0.168 & 72  & 0.0401 \\
3  & 0.515 & 0.348 & 0.40 & 0.168 & 72  & 0.0252 \\
4  & 0.533 & 0.353 & 0.40 & 0.180 & 77  & 0.0196 \\
5  & 0.540 & 0.383 & 0.36 & 0.158 & 68  & 0.0178 \\
6  & 0.580 & 0.388 & 0.38 & 0.193 & 81  & 0.0137 \\
7  & 0.540 & 0.370 & 0.42 & 0.170 & 73  & 0.0123 \\
8  & 0.595 & 0.433 & 0.38 & 0.163 & 76  & 0.0111 \\
9  & 0.583 & 0.438 & 0.39 & 0.145 & 69  & 0.0115 \\
10 & 0.603 & 0.468 & 0.37 & 0.135 & 61  & 0.0101 \\
11 & 0.573 & 0.445 & 0.40 & 0.128 & 62  & 0.0120 \\
12 & 0.608 & 0.490 & 0.38 & 0.118 & 59  & 0.0084 \\
13 & 0.585 & 0.493 & 0.37 & 0.093 & 48  & 0.0087 \\
14 & 0.628 & 0.520 & 0.35 & 0.108 & 57  & 0.0067 \\
15 & 0.608 & 0.500 & 0.39 & 0.108 & 54  & 0.0074 \\
16 & 0.613 & 0.530 & 0.36 & 0.083 & 47  & 0.0055 \\
17 & 0.620 & 0.508 & 0.38 & 0.113 & 58  & 0.0049 \\
18 & 0.653 & 0.535 & 0.38 & 0.118 & 62  & 0.0031 \\
19 & 0.650 & 0.538 & 0.36 & 0.113 & 61  & 0.0039 \\
20 & 0.658 & 0.535 & 0.37 & 0.123 & 62  & 0.0023 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Mistral-7B-Instruct-v0.3 baseline (500 samples, self-score filter). Immediate lock-in regime.}
\label{tab:app-mistral}
\small
\begin{tabular}{rcccccc}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.625 & 0.045 & 0.08 & 0.580 & 232 & 0.3967 \\
2  & 0.635 & 0.065 & 0.05 & 0.570 & 231 & 0.1360 \\
3  & 0.618 & 0.035 & 0.10 & 0.583 & 235 & 0.0956 \\
4  & 0.600 & 0.060 & 0.08 & 0.540 & 219 & 0.0756 \\
5  & 0.618 & 0.063 & 0.09 & 0.555 & 223 & 0.0645 \\
6  & 0.678 & 0.053 & 0.08 & 0.625 & 250 & 0.0604 \\
7  & 0.708 & 0.065 & 0.08 & 0.643 & 259 & 0.0546 \\
8  & 0.713 & 0.065 & 0.11 & 0.648 & 262 & 0.0504 \\
9  & 0.750 & 0.058 & 0.08 & 0.693 & 278 & 0.0472 \\
10 & 0.773 & 0.053 & 0.12 & 0.720 & 288 & 0.0385 \\
11 & 0.803 & 0.048 & 0.11 & 0.755 & 302 & 0.0371 \\
12 & 0.813 & 0.050 & 0.08 & 0.763 & 305 & 0.0304 \\
13 & 0.838 & 0.053 & 0.10 & 0.785 & 316 & 0.0283 \\
14 & 0.883 & 0.050 & 0.07 & 0.833 & 333 & 0.0253 \\
15 & 0.878 & 0.050 & 0.05 & 0.828 & 331 & 0.0248 \\
16 & 0.895 & 0.048 & 0.05 & 0.848 & 339 & 0.0236 \\
17 & 0.898 & 0.050 & 0.11 & 0.848 & 339 & 0.0226 \\
18 & 0.903 & 0.050 & 0.07 & 0.853 & 341 & 0.0204 \\
19 & 0.898 & 0.053 & 0.10 & 0.845 & 338 & 0.0203 \\
20 & 0.900 & 0.050 & 0.05 & 0.850 & 340 & 0.0200 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Llama-3-8B-Instruct baseline (500 samples, self-score filter). Transient peak-recover regime.}
\label{tab:app-llama3}
\small
\begin{tabular}{rcccccc}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.288 & 0.230 & 0.32 & 0.058 & 66  & 0.1280 \\
2  & 0.388 & 0.218 & 0.31 & 0.170 & 89  & 0.0754 \\
3  & 0.450 & 0.203 & 0.28 & 0.248 & 116 & 0.0459 \\
4  & 0.423 & 0.235 & 0.29 & 0.188 & 103 & 0.0351 \\
5  & 0.433 & 0.235 & 0.31 & 0.198 & 106 & 0.0241 \\
6  & 0.468 & 0.240 & 0.27 & 0.228 & 115 & 0.0191 \\
7  & 0.508 & 0.243 & 0.30 & 0.265 & 130 & 0.0161 \\
8  & 0.538 & 0.220 & 0.26 & 0.318 & 138 & 0.0135 \\
9  & 0.558 & 0.235 & 0.25 & 0.323 & 141 & 0.0128 \\
10 & 0.555 & 0.230 & 0.26 & 0.325 & 142 & 0.0093 \\
11 & 0.590 & 0.228 & 0.29 & 0.363 & 152 & 0.0104 \\
12 & 0.588 & 0.233 & 0.33 & 0.355 & 155 & 0.0091 \\
13 & 0.590 & 0.223 & 0.28 & 0.368 & 160 & 0.0079 \\
14 & 0.585 & 0.225 & 0.28 & 0.360 & 159 & 0.0092 \\
15 & 0.583 & 0.230 & 0.29 & 0.353 & 156 & 0.0092 \\
16 & 0.575 & 0.225 & 0.26 & 0.350 & 152 & 0.0081 \\
17 & 0.560 & 0.233 & 0.24 & 0.328 & 145 & 0.0065 \\
18 & 0.503 & 0.233 & 0.21 & 0.270 & 134 & 0.0097 \\
19 & 0.420 & 0.225 & 0.30 & 0.195 & 113 & 0.0152 \\
20 & 0.378 & 0.230 & 0.29 & 0.148 & 92  & 0.0187 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Random negatives (Qwen2.5-7B, 500 samples, $\theta{=}0.0$, inject every 3 iters).}
\label{tab:app-random-neg}
\small
\begin{tabular}{rcccccc}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.493 & 0.320 & 0.38 & 0.173 & 272 & 0.0597 \\
2  & 0.508 & 0.320 & 0.37 & 0.188 & 272 & 0.0368 \\
3  & 0.393 & 0.195 & 0.41 & 0.198 & 322 & 0.2489 \\
4  & 0.458 & 0.333 & 0.35 & 0.125 & 267 & 0.0326 \\
5  & 0.428 & 0.328 & 0.39 & 0.100 & 269 & 0.0328 \\
6  & 0.303 & 0.185 & 0.40 & 0.118 & 326 & 0.1536 \\
7  & 0.423 & 0.338 & 0.37 & 0.085 & 265 & 0.0216 \\
8  & 0.445 & 0.370 & 0.39 & 0.075 & 252 & 0.0171 \\
9  & 0.328 & 0.218 & 0.39 & 0.110 & 313 & 0.1323 \\
10 & 0.465 & 0.375 & 0.42 & 0.090 & 250 & 0.0163 \\
11 & 0.453 & 0.365 & 0.34 & 0.088 & 254 & 0.0183 \\
12 & 0.338 & 0.198 & 0.35 & 0.140 & 321 & 0.0996 \\
13 & 0.460 & 0.363 & 0.37 & 0.098 & 255 & 0.0172 \\
14 & 0.498 & 0.383 & 0.37 & 0.115 & 247 & 0.0178 \\
15 & 0.395 & 0.238 & 0.41 & 0.158 & 305 & 0.1454 \\
16 & 0.503 & 0.405 & 0.42 & 0.098 & 238 & 0.0195 \\
17 & 0.523 & 0.413 & 0.39 & 0.110 & 235 & 0.0145 \\
18 & 0.395 & 0.220 & 0.35 & 0.175 & 312 & 0.1430 \\
19 & 0.493 & 0.393 & 0.36 & 0.100 & 243 & 0.0195 \\
20 & 0.505 & 0.410 & 0.35 & 0.095 & 236 & 0.0234 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Llama-3-8B injection (500 samples, inject every 3 iters, 50\% ratio).}
\label{tab:app-llama3-inj}
\small
\begin{tabular}{rcccccc}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.263 & 0.230 & 0.26 & 0.033 & 59  & 0.1245 \\
2  & 0.438 & 0.193 & 0.26 & 0.245 & 118 & 0.0701 \\
3  & 0.520 & 0.170 & 0.24 & 0.350 & 156 & 0.5000 \\
4  & 0.240 & 0.235 & 0.27 & 0.005 & 56  & 0.1213 \\
5  & 0.333 & 0.255 & 0.31 & 0.078 & 77  & 0.0752 \\
6  & 0.558 & 0.193 & 0.27 & 0.365 & 172 & 0.4616 \\
7  & 0.450 & 0.253 & 0.31 & 0.198 & 106 & 0.0837 \\
8  & 0.495 & 0.303 & 0.30 & 0.193 & 105 & 0.0560 \\
9  & 0.700 & 0.250 & 0.29 & 0.450 & 187 & 0.4541 \\
10 & 0.455 & 0.285 & 0.31 & 0.170 & 94  & 0.0523 \\
11 & 0.485 & 0.288 & 0.28 & 0.198 & 114 & 0.0401 \\
12 & 0.630 & 0.203 & 0.33 & 0.428 & 192 & 0.4601 \\
13 & 0.553 & 0.288 & 0.32 & 0.265 & 133 & 0.0313 \\
14 & 0.565 & 0.285 & 0.30 & 0.280 & 136 & 0.0278 \\
15 & 0.745 & 0.150 & 0.28 & 0.595 & 250 & 0.4264 \\
16 & 0.583 & 0.278 & 0.32 & 0.305 & 145 & 0.0374 \\
17 & 0.670 & 0.280 & 0.33 & 0.390 & 169 & 0.0278 \\
18 & 0.780 & 0.180 & 0.30 & 0.600 & 255 & 0.4030 \\
19 & 0.673 & 0.290 & 0.24 & 0.383 & 167 & 0.0289 \\
20 & 0.635 & 0.315 & 0.24 & 0.320 & 151 & 0.0192 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Baseline 500 (Qwen2.5-7B, 500 samples, self-score filter).}
\label{tab:app-baseline500}
\small
\begin{tabular}{rcccccc}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.493 & 0.320 & 0.39 & 0.173 & 75  & 0.0598 \\
2  & 0.498 & 0.295 & 0.37 & 0.203 & 85  & 0.0366 \\
3  & 0.510 & 0.323 & 0.37 & 0.188 & 79  & 0.0226 \\
4  & 0.503 & 0.308 & 0.37 & 0.195 & 85  & 0.0177 \\
5  & 0.550 & 0.340 & 0.37 & 0.210 & 89  & 0.0147 \\
6  & 0.545 & 0.350 & 0.42 & 0.195 & 83  & 0.0157 \\
7  & 0.558 & 0.330 & 0.36 & 0.228 & 92  & 0.0143 \\
8  & 0.545 & 0.338 & 0.36 & 0.208 & 87  & 0.0146 \\
9  & 0.588 & 0.355 & 0.41 & 0.233 & 97  & 0.0171 \\
10 & 0.605 & 0.360 & 0.41 & 0.245 & 98  & 0.0157 \\
11 & 0.615 & 0.373 & 0.41 & 0.243 & 101 & 0.0105 \\
12 & 0.638 & 0.368 & 0.40 & 0.270 & 110 & 0.0085 \\
13 & 0.633 & 0.368 & 0.38 & 0.265 & 108 & 0.0085 \\
14 & 0.668 & 0.378 & 0.37 & 0.290 & 118 & 0.0070 \\
15 & 0.668 & 0.378 & 0.43 & 0.290 & 120 & 0.0059 \\
16 & 0.678 & 0.380 & 0.39 & 0.298 & 122 & 0.0060 \\
17 & 0.693 & 0.383 & 0.43 & 0.310 & 129 & 0.0038 \\
18 & 0.693 & 0.373 & 0.42 & 0.320 & 131 & 0.0054 \\
19 & 0.708 & 0.373 & 0.41 & 0.335 & 136 & 0.0034 \\
20 & 0.695 & 0.375 & 0.42 & 0.320 & 130 & 0.0040 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Recovery (Qwen2.5-7B, 500 samples, injection every iter from collapsed checkpoint).}
...
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.713 & 0.368 & 0.38 & 0.345 & 138 & 0.0063 \\
2  & 0.758 & 0.248 & 0.44 & 0.510 & 205 & 0.2462 \\
3  & 0.460 & 0.225 & 0.44 & 0.235 & 99  & 0.2175 \\
4  & 0.560 & 0.300 & 0.44 & 0.260 & 121 & 0.2779 \\
5  & 0.578 & 0.328 & 0.45 & 0.250 & 116 & 0.2270 \\
6  & 0.638 & 0.390 & 0.49 & 0.248 & 118 & 0.1989 \\
7  & 0.553 & 0.325 & 0.48 & 0.228 & 124 & 0.1670 \\
8  & 0.645 & 0.325 & 0.42 & 0.320 & 146 & 0.1835 \\
9  & 0.610 & 0.373 & 0.46 & 0.238 & 125 & 0.1561 \\
10 & 0.575 & 0.320 & 0.46 & 0.255 & 130 & 0.1541 \\
11 & 0.615 & 0.383 & 0.42 & 0.233 & 129 & 0.1404 \\
12 & 0.520 & 0.475 & 0.37 & 0.045 & 83  & 0.1231 \\
13 & 0.560 & 0.380 & 0.35 & 0.180 & 111 & 0.1343 \\
14 & 0.545 & 0.415 & 0.34 & 0.130 & 103 & 0.1360 \\
15 & 0.478 & 0.425 & 0.33 & 0.053 & 71  & 0.1440 \\
16 & 0.510 & 0.443 & 0.29 & 0.068 & 78  & 0.1544 \\
17 & 0.578 & 0.375 & 0.37 & 0.203 & 112 & 0.1232 \\
18 & 0.543 & 0.418 & 0.36 & 0.125 & 96  & 0.1182 \\
19 & 0.553 & 0.353 & 0.38 & 0.200 & 112 & 0.1399 \\
20 & 0.565 & 0.363 & 0.37 & 0.203 & 117 & 0.1386 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Full fine-tuning baseline (Qwen2.5-7B, 500 samples, all 7.6B params trainable, no LoRA).}
\label{tab:app-full-ft}
\small
\begin{tabular}{rcccccc}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.495 & 0.313 & 0.38 & 0.183 & 75  & 0.0763 \\
2  & 0.523 & 0.330 & 0.46 & 0.193 & 84  & 0.0390 \\
3  & 0.565 & 0.350 & 0.35 & 0.215 & 90  & 0.0269 \\
4  & 0.555 & 0.355 & 0.45 & 0.200 & 85  & 0.0186 \\
5  & 0.623 & 0.358 & 0.40 & 0.265 & 109 & 0.0157 \\
6  & 0.610 & 0.338 & 0.37 & 0.273 & 110 & 0.0161 \\
7  & 0.635 & 0.368 & 0.40 & 0.268 & 107 & 0.0217 \\
8  & 0.573 & 0.325 & 0.41 & 0.248 & 100 & 0.0236 \\
9  & 0.633 & 0.370 & 0.37 & 0.263 & 106 & 0.0169 \\
10 & 0.643 & 0.363 & 0.43 & 0.280 & 117 & 0.0156 \\
11 & 0.665 & 0.380 & 0.40 & 0.285 & 115 & 0.0132 \\
12 & 0.660 & 0.358 & 0.39 & 0.303 & 123 & 0.0102 \\
13 & 0.685 & 0.390 & 0.35 & 0.295 & 119 & 0.0075 \\
14 & 0.693 & 0.370 & 0.39 & 0.323 & 130 & 0.0060 \\
15 & 0.715 & 0.385 & 0.36 & 0.330 & 132 & 0.0057 \\
16 & 0.710 & 0.370 & 0.39 & 0.340 & 137 & 0.0077 \\
17 & 0.738 & 0.378 & 0.34 & 0.360 & 144 & 0.0050 \\
18 & 0.703 & 0.375 & 0.37 & 0.328 & 132 & 0.0067 \\
19 & 0.743 & 0.383 & 0.37 & 0.360 & 146 & 0.0034 \\
20 & 0.718 & 0.380 & 0.39 & 0.338 & 136 & 0.0043 \\
\bottomrule
\end{tabular}
\end{table}

\section{Seed Replication Data}
\label{app:seed43}

Tables~\ref{tab:app-baseline-seed43} and~\ref{tab:app-injection-seed43} report
the full per-iteration trajectories for the seed-43 replication runs referenced
in Section~\ref{sec:seed-replication} (Qwen2.5-7B, 250 samples, 20 iterations).

\begin{table}[h]
\centering
\caption{Baseline, seed 43 (Qwen2.5-7B, 250 samples, self-score filter).}
\label{tab:app-baseline-seed43}
\small
\begin{tabular}{rcccccc}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ \\
\midrule
1  & 0.480 & 0.350 & 0.46 & 0.130 & 31 & 0.0650 \\
2  & 0.530 & 0.400 & 0.38 & 0.130 & 29 & 0.0369 \\
3  & 0.550 & 0.400 & 0.38 & 0.150 & 35 & 0.0256 \\
4  & 0.615 & 0.410 & 0.40 & 0.205 & 43 & 0.0175 \\
5  & 0.635 & 0.420 & 0.42 & 0.215 & 43 & 0.0172 \\
6  & 0.640 & 0.415 & 0.40 & 0.225 & 48 & 0.0147 \\
7  & 0.620 & 0.415 & 0.42 & 0.205 & 41 & 0.0129 \\
8  & 0.670 & 0.430 & 0.38 & 0.240 & 50 & 0.0098 \\
9  & 0.690 & 0.430 & 0.38 & 0.260 & 54 & 0.0072 \\
10 & 0.740 & 0.440 & 0.34 & 0.300 & 60 & 0.0064 \\
11 & 0.715 & 0.450 & 0.36 & 0.265 & 54 & 0.0040 \\
12 & 0.740 & 0.450 & 0.38 & 0.290 & 59 & 0.0035 \\
13 & 0.735 & 0.455 & 0.38 & 0.280 & 57 & 0.0021 \\
14 & 0.735 & 0.455 & 0.36 & 0.280 & 56 & 0.0016 \\
15 & 0.740 & 0.455 & 0.38 & 0.285 & 57 & 0.0016 \\
16 & 0.750 & 0.460 & 0.38 & 0.290 & 59 & 0.0010 \\
17 & 0.740 & 0.460 & 0.36 & 0.280 & 57 & 0.0011 \\
18 & 0.750 & 0.460 & 0.36 & 0.290 & 59 & 0.0010 \\
19 & 0.755 & 0.460 & 0.36 & 0.295 & 59 & 0.0012 \\
20 & 0.760 & 0.465 & 0.38 & 0.295 & 59 & 0.0012 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Injection, seed 43 (Qwen2.5-7B, 250 samples, inject every 3 iters, 50\% ratio).
$\bigstar$ = injection iteration.}
\label{tab:app-injection-seed43}
\small
\begin{tabular}{rccccccl}
\toprule
Iter & $\overline{\self}$ & $\overline{\gt}_\text{tr}$ & $\overline{\gt}_\text{val}$ & $\gap$ & HN & $\mathcal{L}$ & \\
\midrule
1  & 0.480 & 0.350 & 0.40 & 0.130 & 31 & 0.0651 & \\
2  & 0.550 & 0.365 & 0.38 & 0.185 & 40 & 0.0369 & \\
3  & 0.600 & 0.310 & 0.40 & 0.290 & 62 & 0.2650 & $\bigstar$ \\
4  & 0.565 & 0.415 & 0.42 & 0.150 & 31 & 0.0268 & \\
5  & 0.500 & 0.385 & 0.40 & 0.115 & 26 & 0.0225 & \\
6  & 0.540 & 0.220 & 0.44 & 0.320 & 66 & 0.2666 & $\bigstar$ \\
7  & 0.540 & 0.395 & 0.42 & 0.145 & 32 & 0.0196 & \\
8  & 0.540 & 0.430 & 0.40 & 0.110 & 28 & 0.0219 & \\
9  & 0.730 & 0.320 & 0.42 & 0.410 & 82 & 0.1846 & $\bigstar$ \\
10 & 0.545 & 0.415 & 0.42 & 0.130 & 30 & 0.0147 & \\
11 & 0.590 & 0.410 & 0.44 & 0.180 & 37 & 0.0141 & \\
12 & 0.735 & 0.285 & 0.44 & 0.450 & 91 & 0.1680 & $\bigstar$ \\
13 & 0.630 & 0.435 & 0.44 & 0.195 & 40 & 0.0096 & \\
14 & 0.635 & 0.465 & 0.42 & 0.170 & 40 & 0.0121 & \\
15 & 0.740 & 0.330 & 0.38 & 0.410 & 94 & 0.1107 & $\bigstar$ \\
16 & 0.655 & 0.465 & 0.40 & 0.190 & 41 & 0.0072 & \\
17 & 0.660 & 0.490 & 0.36 & 0.170 & 37 & 0.0138 & \\
18 & 0.750 & 0.400 & 0.40 & 0.350 & 77 & 0.1479 & $\bigstar$ \\
19 & 0.695 & 0.525 & 0.40 & 0.170 & 38 & 0.0127 & \\
20 & 0.690 & 0.525 & 0.42 & 0.165 & 39 & 0.0182 & \\
\bottomrule
\end{tabular}
\end{table}
